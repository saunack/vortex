...
int main(int argc, char *argv[]) {
  ...
  uint32_t buf_src_size = num_points * sizeof(int8_t);
  uint32_t buf_dst_size = num_points * sizeof(int32_t);
  ...
  std::vector<int8_t> h_A(num_points);
  std::vector<int8_t> h_B(num_points);
  std::vector<int32_t> h_C(num_points);
  for (uint32_t i = 0; i < num_points; ++i) {
    auto a = static_cast<float>(std::rand()) / RAND_MAX;
    auto b = static_cast<float>(std::rand()) / RAND_MAX;
    h_A[i] = static_cast<int8_t>(a * size);
    h_B[i] = static_cast<int8_t>(b * size);
  }
